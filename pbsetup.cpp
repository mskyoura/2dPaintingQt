                ...
                if (donor)
                {
                    ...
                    switch (cmdType) {
                        ...
                        case _RELAY2ON:
                            ...
                            {
                                int delay = donor->_Delay() * 10;
                                QString t1 = pWin->Usb->byteToQStr(donor->_Duration());
                                QString t2 = pWin->Usb->byteToQStr((delay & 0xFF00)>>8) + pWin->Usb->byteToQStr(delay & 0x00FF);
                                r2On = "01" + t1;
                                r2Del = t2;
                            }
                            break;
...
QString PBsetup::buildSingleCommand(const QString& deviceId, CmdTypes cmdType, const QString& iCmdNum,
    ...
    if (cmdType == _STATUS) {
        ...
        if (cmdType == _RELAY2ON) {
            // Order: Duration (T1) then Delay (T2)
            cmdRq += "01" + t1 + t2;
        } else {
            cmdRq += "FFFFFFFF";
        }
    ...
    switch (cmdType) {
        case _RELAY2ON:
            return QString("№ %1, %2, %3")
                   .arg(finalCmdNum)
                   .arg(donor.getDuration(true))
                   .arg(donor.getDelay(true));
        case _RELAY1ON:
        case _RELAY1OFF:
...
void PBsetup::scheduleTwoStageRelayTransition(const QString& id, const QList<int>& donorsNum, Saver* donor)
{
    ...
    int t2DelayMs = donor->_Delay() * 1000; // T2 в миллисекундах
    if (t2DelayMs > 0) {
            ...
            for (int vmIndex : donorsNum) {
                ...
                if (targetDonor->_ID() == id) {
                    if (canScheduleStatusChange(targetDonor, RELAY2ON)) {
                        ...
                        int t1DelayMs = donor->_Duration() * 1000; // T1 в миллисекундах
                        if (t1DelayMs > 0) {
...
int PBsetup::computeStatusChangeDelayMs(Saver* donor, CmdTypes cmdType, RelayStatus statusToSet)
{
    ...
    case _RELAY2ON:
        if (statusToSet == RELAY1ON && donor && donor->getLastStatus() == RELAY2ON) {
            ...
            int delayT1 = donor ? donor->_Duration() : 0;
...
{
    pWin->appset->set(donor._ID(), donor.getDst(), donor._Duration(), donor._Delay(),
...
void PBsetup::updateDonorFromSettings(Saver& donor, const QString& idBefore)
{
    ...
    donor.setDuration(pWin->appset->T1);
    donor.setDelay(pWin->appset->T2);
